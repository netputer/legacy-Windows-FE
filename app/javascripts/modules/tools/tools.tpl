...
<script type="text/x-ui-template" id="tools-falsh-card">
    <div class="icon"></div>
    <div class="info-ctn vbox">
        <div class="top-ctn hbox">
            <h1 class="title">{{= i18n.tools.FALSH_TITLE }}</h1>
            <button class="action button-action">{{= i18n.tools.FALSH_DEVICE }}</button>
            <button class="normal button-cancel">{{= i18n.tools.FALSH_CANCEL }}</button>
        </div>
        <div class="desc text-thirdly">{{= i18n.tools.FALSH_DEVICE_DESC }}</div>
        <progress class="tiny lighter" max="120" value="0"></progress>
    </div>
</script>

<script type="text/x-ui-template" id="tools-falsh-confirm">
    <div class="w-tools-falsh-confirm hbox">
        <div class="icon"></div>
        <div class="desc-ctn">
            <h1 class="title text-secondary">{{= i18n.tools.CONFIRM_TITLE }}</h1>
            <span class="dsec text-thirdly">{{= i18n.tools["FALSH_ERROR_" + it.error] }}</span>
        </div>
    </div>
</script>
